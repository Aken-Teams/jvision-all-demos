...
- [零、方向：只留一條對外路徑（決策中）](#零方向只留一條對外路徑決策中)
...
| B：`c-<公司>-<系統>.jvision-ai.com` | **我們的**（同一條 tunnel） | ✅ 已修好並改成**按下佈署才建 DNS**（2026-09-03，見[問題 8](#8-c-xxxjvision-aicom-子網域從來沒有能用過-已修2026-09-03)） |
...
### 8. `c-xxx.jvision-ai.com` 子網域從來沒有能用過　✅ 已修（2026-09-03）
...
### 9. 登入的 `next` 參數是開放重導　✅ 已修（2026-09-03）
...
### 前提：先定案[第零節](#零方向只留一條對外路徑決策中)

砍不砍 Vercel、留不留 `c-xxx` 子網域，決定了下面一半的項目要不要做。
**這一題沒定之前，不要動第 3、6、7 項**——那三項都是在為某一條路徑投資。

### 假設「砍 Vercel、以 Cloudflare 為主」成立

| 順位 | 做什麼 | 為什麼 | 大約成本 |
|---|---|---|---|
| 1 | **封存功能** | 使用者現在就要，可逆，零外部依賴，後端幾乎已經寫好 | 半天 |
| 2 | **問題 9**：`next` 開放重導加白名單 | 五行，而且是修問題 8 的前置 | 十分鐘 |
| 3 | **停止建新的 DNS**（拿掉 `routeDns()` 呼叫） | 前端本來就沒用那個網址，近乎零風險 | 十分鐘 |
| 4 | **問題 2**：公開版唯讀 或 寫入加 token | 只要 Vercel 還在跑一天就成立 | 半天 |
| 5 | **決定 B 的去留**：修好子網域，或全部走 `/-/i/<id>/` | 老闆在意的「像不像交付」就是這一題 | 討論 ＋ 半天 |
| 6 | **問題 8**：修子網域的三個阻擋（若 5 選了「修」） | 不修的話那 25 筆 DNS 永遠是死的 | 半天 |
| 7 | **移除 Vercel**：拿掉按鈕、清 Vercel 專案、DROP 所有 `_pub` 資料庫 | 問題 1／3／4 會一起消失 | 半天 ＋ 人工清理 |
| 8 | **硬刪除功能** | 要拆的東西在第 7 項之後少一半 | 一天 |
| 9 | **問題 6**：GitHub 交付的文案或行為 | 現在是誤導，但沒有立即風險 | 看選哪條路 |
| 10 | **問題 7**：資料庫搬遷 | 等架構定案一起規劃 | 依範圍 |

### 如果決定保留 Vercel

那 [問題 1](#1-公開部署的憑證可以觸及整個平台)（`jv_pub` 專用帳號，半小時）
就變成**最優先**，因為它是唯一一項「單點失守會波及所有客戶」的；
接著是[問題 4](#4-vercel-專案名跨客戶撞名)（專案名撞名，是刪除功能的前置條件）
與[問題 3](#3-公開的是真實資料的複本)（資料揭露的告知）。

### 不論走哪條

- 第 1 項（封存）與第 2 項（`next` 白名單）都是無條件該做的，不依賴任何決定
