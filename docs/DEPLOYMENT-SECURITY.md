# 從開發到部署：完整流程、資源影響與待處理事項

> 盤點日期 2026-09-03，同日兩次更新（第二次重新核對了每一項的現況）。
> 每一項都附了檔案與行號，可以逐項打開對照。
>
> 這份文件回答三個問題：
> 1. 使用者按下一顆按鈕之後，**實際上在幾個地方留下了東西**
> 2. 那些東西分別**掛在誰的帳上**（我們的主機／Cloudflare／Vercel／GitHub／MySQL）
> 3. 因此**刪除功能要拆掉什麼**、**資源要怎麼規劃**、**哪些事情要先做**

---

## 進度：做完了什麼、還剩什麼

下面每一格都是**重新跑過指令核對**的，不是憑印象。

### 已完成

| 項目 | 做了什麼 | 驗證方式 |
|---|---|---|
| 子網域三個阻擋（問題 8） | 登入一律導主站、90 秒交接票換發 host-only cookie、redirect_uri 固定 | 未登入打子網域 3 跳直達 Google；持票換 cookie 後拿到 HTTP 200 |
| `next` 開放重導（問題 9） | `safeNext()`：只認站內相對路徑與資料庫裡真的存在的實例子網域 | `to=https://evil.com`、`to=//evil.com` 都導不出去 |
| 子網域不再預先存在 | DNS 從「開通時建」改成「按下佈署才建」（`lib/instance-dns.mjs`） | 開通流程已無 `routeDns`；佈署／取消佈署走完並確認 DNS 增減 |
| 清掉沒人用的 DNS | 刪 26 筆（23 筆從未佈署 ＋ 3 筆查無實例的殘骸） | zone 48 → 22 筆，公司的 20 筆服務一筆沒動；備份在 `var/backup/` |
| 佈署 ／ 取消佈署 ／ 完整度檢查 | 交付頁三張卡（佈署、分享、交付），只有擁有者能改 | 非擁有者查狀態與按佈署都 403 |
| 使用名單看得到、改得動 | 列出成員＋角色，可移除，分頁 | 加入→取消→移除全程；擁有者不給移除鈕 |
| 交付不再毀掉 git 歷史 | 先 fetch 遠端再接上去，不再 `git init` 重來、不再無條件 force | 對真實 repo：commit 1→2、新 commit 父數 1、diff 只有實際改動 |
| 交付第二次必定失敗 | 認出「拿掉 CI 檔之後與遠端相同」的空提交，回報「沒有東西要更新」 | 同一分支連推兩次，第二次正確回無變更 |
| 「開 PR」是假的 | 那顆按鈕需要 token 有 pull requests 權限，沒有時只印警告卻回成功。改成**選分支／建分支推上去**，推完給 compare 連結 | 資料庫裡 3 次交付紀錄的 url 全是 repo 網址、沒有一個是 PR |
| Vercel 入口 | 畫面上藏起來（`workspace.html` 那張卡 `hidden`），程式碼保留 | 交付頁只剩三張卡 |

### 還沒做（按嚴重度）

| 項目 | 現況核對結果 | 為什麼還沒消失 |
|---|---|---|
| **[問題 1](#1-公開部署的憑證可以觸及整個平台) 憑證** | ❗ **仍然成立，而且是活的** | 見下面那個警告框 |
| **[問題 2](#2-公開網址可以刪光資料沒有任何身分驗證) 公開版可寫可刪** | ❗ 仍然成立 | `nextjs-bundle.mjs` 的 POST/PATCH/DELETE 還在（:212／:230／:245），整份檔案沒有出現 `authorization` |
| **[問題 3](#3-公開的是真實資料的複本) 公開真資料** | 仍然成立 | `mirrorDatabase()` 還是 `CREATE TABLE LIKE` ＋ `INSERT SELECT`（:54-55） |
| **[問題 4](#4-vercel-專案名跨客戶撞名) 專案名撞名** | 仍然成立 | `instance-deploy-vercel.mjs:105` 的專案名還是不含實例編號 |
| **[問題 5](#5-沒有任何外部資源的回收路徑) 回收路徑** | 部分改善 | `destroyInstance()` 只 `DROP DATABASE` ＋ 刪一列；DNS 現在**有**回收函式（`dns.unpublish`）但**沒有人呼叫它** |
| **問題 6 GitHub 文案** | 文案已修，定位待老闆確認 | 卡片已改成「推成一個 repo」，不再說「你自己的」。repo 仍在我們組織下——那現在是**刻意的**（老闆可能要收開發者維運費），等確認 |
| **[問題 7](#7-所有資料庫住在同一台伺服器) 同一台資料庫** | 仍然成立 | 未動 |
| **[問題 10](#10-交付用的-github-token-是個人帳號權杖) GitHub token 權限過大** | 🆕 這次新發現 | 見該節 |
| 封存功能 | **完全沒做** | `instances.archived_at` 欄位存在、`state='archived'` 判斷存在，但沒有任何 API 與畫面 |

> ### ❗ 最該先處理的一件事
>
> 「把 Vercel 按鈕藏起來」**沒有讓已經部署出去的東西消失**。這次實測：
>
> ```
> https://jv-inventory.vercel.app                        HTTP 200
> https://jv-construction-insurance-reconcili.vercel.app HTTP 200
> ```
>
> 兩個站都還活著，`/_jv/schema` 與 `/api/t/table_1` **不需要任何身分驗證**就讀得到
> 真實資料（分別 1 筆與 6 筆），而產生器同時產出了 POST／PATCH／DELETE。
> 伺服器上也還留著 3 個 `_pub` 資料庫
> （`jv_imtidi0ezdffaa5_pub`、`jv_imtiljt16691b85_pub`、`jv_imtiq1fi19140c2_pub`）。
>
> 更要緊的是那兩個站的環境變數裡放著 `.env` 原封不動送過去的 `MYSQL_USER`／
> `MYSQL_PASSWORD`——也就是 A999，而 A999 連得到 `jv_demo_case`（客戶、成員、
> 訂單、對話）與每一個 `jv_<實例>`。
>
> **藏按鈕擋的是「未來不要再發生」，不是「已經發生的」。** 要嘛把這兩個
> Vercel 專案刪掉、DROP 那 3 個 `_pub`，要嘛立刻把憑證換成
> [問題 1](#1-公開部署的憑證可以觸及整個平台) 說的 `jv_pub` 專用帳號。

---

## 目錄

- [進度：做完了什麼、還剩什麼](#進度做完了什麼還剩什麼)
- [零、方向：只留一條對外路徑](#零方向只留一條對外路徑決策中)
- [一、全景：一套系統有四種存在形式](#一全景一套系統有四種存在形式)
- [二、流程 ①　模板複製（開通）](#二流程-模板複製開通)
- [三、流程 ②　在工作台修改](#三流程-在工作台修改)
- [四、流程 ③　分享給同事](#四流程-分享給同事)
- [五、流程 ④　部署到 Vercel](#五流程-部署到-vercel)
- [六、流程 ⑤　交付到 GitHub](#六流程-交付到-github)
- [七、資源總表：一個實例最多散落在 11 個地方](#七資源總表一個實例最多散落在-11-個地方)
- [八、資源規劃：MySQL／Cloudflare／Vercel／GitHub](#八資源規劃mysqlcloudflarevercelgithub)
- [九、待處理問題](#九待處理問題)
- [十、刪除功能該怎麼做](#十刪除功能該怎麼做)
- [十一、建議順序](#十一建議順序)

---

## 零、方向：只留一條對外路徑（決策中）

> 狀態：**部分定案**（2026-09-03）

> **這一節的狀態變了。** 原本三條路都在跑、沒有人決定要留哪一條。現在：
>
> - **A（路徑式入口）** 仍然是主力，所有分享與工作台預覽都走這條。
> - **B（`c-xxx` 子網域）** 修好了，並且改成**按下「佈署」才建 DNS** ——
>   使用者沒按，公開 DNS 上就查不到任何屬於他的網址。這是佈署按鈕現在走的路。
> - **C（Vercel）** 畫面上藏起來、程式碼保留。老闆可能認為 Vercel 與 Cloudflare
>   的產品定位不同，所以是「先隱藏」不是「刪除」。
>   ⚠️ **但藏按鈕不等於收乾淨**：兩個既有的 Vercel 站還活著、還公開可讀、
>   環境變數裡還放著最大權限的 MySQL 憑證。見開頭那個警告框。
>
> 還沒定的是：GitHub repo 要放我們組織還是客戶自己名下（老闆的維運費模式會
> 決定這一題，見問題 6）。



> 2026-09-03 的初步結論，**尚未與老闆確認**。寫在這裡是為了讓後面每一項改動
> 都有一個共同的前提，不是已經定案。

### 現況：同一件事有三條路，而且沒有人打算讓它們共存

| 路 | 誰的機器 | 現在的狀態 |
|---|---|---|
| A：`jvdemo.jvision-ai.com/-/i/<id>/` | **我們的**（cloudflared tunnel） | ✅ 唯一真正在跑的。分享、開啟、工作台預覽全走這條 |
| B：`c-<公司>-<系統>.jvision-ai.com` | **我們的**（同一條 tunnel） | ✅ 已修好並改成**按下佈署才建 DNS**（2026-09-03，見[問題 8](#8-c-xxxjvision-aicom-子網域從來沒有能用過)） |
| C：Vercel | **Vercel 的** | ⚠️ 能用，但整套重複：複製一份資料庫、把 MySQL 帳密送到第三方 |

A 與 B 其實是**同一條 tunnel 的兩種網址寫法**，差別只有好不好看；
C 才是真正的第二套基礎設施。

### 決定

**砍掉 C（Vercel），以 Cloudflare 為主。**

理由：

- 主站本來就跑在 tunnel 上，A 不是新增依賴，是**已經在用的東西**
- `var/instances/<id>/public/` 已經由 `app-server.mjs` 在服務，路由 `/-/i/<id>/` 已經在跑——
  要「讓外面的人看到」，需要的東西全部都已經存在了
- C 唯一多給的是「免登入的公開連結」與「我們關機它還活著」。
  前者可以在 gateway 上用一組 share token 做（同一台機器、同一份資料、可撤銷、
  可設期限、唯讀）；後者是託管方案，是產品與收費決定，不該混在一顆「分享」按鈕裡

成本取捨（這是選 A/B 而不選 C 的代價，要認）：

| | Cloudflare（A/B） | Vercel（C） |
|---|---|---|
| 雲端費用 | ≈ 0 | 隨用量成長 |
| 吃什麼 | **我們自己機器的 CPU／記憶體／磁碟** | 不吃 |
| 我們關機 | 全部離線 | 還活著 |
| 資料新鮮度 | 永遠最新 | 部署那一刻的快照 |
| MySQL 帳密外送 | 不用 | **要** |

### B 要不要留

`/-/i/i_mtfpybt8_505f80/` 這種網址確實不像交付出去的東西，
所以 B 存在的理由是**體感與品牌**，不是技術。這個理由成立。

但 B 現在是壞的，要留就得先修好[問題 8](#8-c-xxxjvision-aicom-子網域從來沒有能用過)，
而修好的代價是把 cookie 放寬到整個 `.jvision-ai.com`——那有它自己的風險。

兩個選項：

- **先全部走 A**，把 B 的 DNS 停掉。最省，而且今天就沒有人在用
- **修好 B 再全面改用 B**，A 退回成內部備援。網址好看，但要付出 cookie 放寬的代價

**不論選哪個，都不要兩個都留著長期並存**——那正是現在的狀態，也正是沒有人
說得出「部署到底是什麼」的原因。

---

## 一、全景：一套系統有四種存在形式

同一套系統在不同階段是**四份不同的東西**，很容易混淆，先把它們分清楚：

| # | 名字 | 位置 | 資料 | 誰看得到 | 誰建立的 |
|---|---|---|---|---|---|
| 1 | **型錄 demo** | `demos/<repo>/index.html` | 無（寫死的假資料） | 全世界（型錄站） | agent 產線 |
| 2 | **實例** | `var/instances/<id>/` ＋ MySQL `jv_<id>` | 客戶的真實資料 | Google 登入 ＋ 該公司白名單 | 模板複製 |
| 3 | **公開版** | Vercel ＋ MySQL `jv_<id>_pub` | 真實資料的**複本** | **任何拿到連結的人** | 「部署到 Vercel」 |
| 4 | **交付版** | GitHub `JVision-pj/<repo>-<id6>` | 無（只有結構與種子） | 我們的 GitHub 組織成員 | 「交付到 GitHub」 |

**關鍵界線**：第 1 份是唯讀展示品，`instance-bind.mjs:56-63` 有一道硬性檢查，
任何寫入路徑落在 `demos/` 底下都會直接丟例外——不是靠註解自律。
第 2～4 份都是它的下游產物，改哪一份都不會回頭汙染型錄。

---

## 二、流程 ①　模板複製（開通）

> **這是使用者按「模板複製」時真正發生的事。**
> 很多人以為 DNS／子網域是部署階段才發生的——**不是，是這一步。**

### 觸發路徑

```
catalog.html 卡片上的「模板複製」
  → app.js:517         POST /api/templates/copy { repoName }
  → dev.mjs:415-446    建 customer + order，然後 execFileSync
  → tools/instance-provision.mjs --order=<單號>
```

### 逐步做了什麼

| 步驟 | 程式 | 建立的東西 | 在誰的帳上 | 可回收？ |
|---|---|---|---|---|
| 1 | `dev.mjs:428` | `customers` 一列（若首次） | 我們的 MySQL | 沒有程式 |
| 2 | `dev.mjs:429` | `orders` 一列（`note: 從目錄一鍵複製`） | 我們的 MySQL | 沒有程式 |
| 3 | `instance-provision.mjs:37-44` | 決定子網域 `c-<公司>-<系統>.jvision-ai.com` | — | — |
| 4 | `instance-provision.mjs:85` | **`instances` 一列**（含 `host`、`db_name`、`dir`） | 我們的 MySQL | ✅ `destroyInstance()` |
| 5 | `instance-bind.mjs:76-105` | **檔案目錄 `var/instances/<id>/`** | 我們的主機磁碟 | ❌ 只有 `instance-verify` 自己清測試用的 |
| 6 | `instance-db.mjs:35` | **專屬資料庫 `jv_<id>`** ＋ 建表 ＋ 灌種子資料 | 我們的 MySQL | ✅ `destroyInstance()` → `DROP DATABASE` |
| 7 | `instance-provision.mjs:55-66` | **Cloudflare DNS 記錄**（`cloudflared tunnel route dns`） | **Cloudflare 公司帳號** | ❌ **完全沒有** |
| 8 | `instance-provision.mjs:95` | `events` 一列 `instance.live` | 我們的 MySQL | ❌ 沒有級聯刪除 |

### `var/instances/<id>/` 裡面到底是什麼

你問的這一點——它**不是**資料庫的 ID 資訊，是**這套系統的網頁本體**：

```
var/instances/i_mtegnuqu_db3c75/
├── public/
│   ├── index.html          ← demo 的 HTML 複本，尾端注入了三支 runtime script
│   └── _jv/
│       ├── live.js         ← 把畫面上的 <table> 接到真資料庫（shared/jv-live.js 的複本）
│       ├── assist.js       ← 右下角的修改助理
│       ├── tour.js         ← 第一次進來的導覽
│       ├── tour.json       ← 導覽要講的內容（標題、欄位名）
│       ├── schema.json     ← 資料表定義的複本，交付時客戶要看得懂自己的結構
│       └── favicon.svg
├── uploads/                ← 客戶在系統裡上傳的檔案（25 個實例中有 5 個有東西）
└── README.md
```

三支 runtime 是**複製**進去而不是連回站台，因為實例交付出去之後是獨立部署的
（`instance-bind.mjs:79-81` 的註解寫得很清楚）。

目前 25 個實例共 5.7MB。**磁碟不是瓶頸，`uploads/` 裡的客戶檔案才是刪除時要注意的**——
那是唯一一份，資料庫裡沒有副本。

### Cloudflare 那一段：你的理解要修正

你問「這是佈署以後他會放到公司 CLOUDFLARE 的 TUNNEL 上？」——

**不是部署以後，是模板複製的當下就建了。** 而且 tunnel 本身不用動，
因為 `~/.cloudflared/jvdemo.yml` 的 ingress 已經有一條萬用規則：

```yaml
- hostname: "*.jvision-ai.com"
  service: http://127.0.0.1:3000
```

所以新增客戶是**零基礎設施動作**——tunnel 不必改、不必重啟。
`instance-provision.mjs` 那一步建的是 **Cloudflare 的 DNS 記錄**（CNAME 指向 tunnel），
只是讓那個子網域「解析得到」。

刻意不用萬用 DNS（`*.jvision-ai.com`）的理由寫在 `instance-provision.mjs:46-52`：
萬用 DNS 會把整個公司網域的任何子網域都指到這條 tunnel，`www` 與 `app`
那些真正的服務只要哪天記錄出問題就會被接管。

**後果**：現在 25 個實例 = Cloudflare 上 25 筆 DNS 記錄，**一筆都沒被回收過**。
而且 `pickHost()` 撞名會加序號（`-2`、`-3`），所以刪掉再重建同一套系統，
網址會變成 `c-xxx-2.jvision-ai.com`，舊那筆繼續留著。

---

## 三、流程 ②　在工作台修改

這一步**不建立任何外部資源**，全部落在我們自己的 MySQL 裡。

| 動作 | 落在哪 |
|---|---|
| 對話 | `jv_demo_case.chat_sessions` / `chat_messages` |
| 改欄位、改名稱 | `jv_<id>` 的實際資料表 ＋ `jv_<id>.jv_audit` |
| 版本快照 | `instance-versions.mjs` |
| 貼截圖 | `var/instances/<id>/uploads/` |

請求怎麼進來（兩個入口共用同一套把關，`dev.mjs:308-330`）：

```
①  https://jvdemo.jvision-ai.com/-/i/<id>/...      內部備援，不必動 DNS
②  https://c-<公司>-<系統>.jvision-ai.com/...      客戶對外用的網址
        ↓  gateway（dev.mjs）驗 Google 身分 ＋ 公司白名單
        ↓  用標頭 x-jv-instance 傳實例身分（不從路徑或 query 取）
   app-server.mjs（綁 127.0.0.1:4700）
```

**這裡的隔離設計是好的**：實例身分只從標頭來，前端傳什麼都跨不到別的實例；
`app-server` 綁 127.0.0.1 且自己再擋一次（縱深防禦）。
兩層都已經認得 `state === "archived"` 並回 410（`dev.mjs:276`、`app-server.mjs:101`）——
這一點對後面的刪除功能很重要。

---

## 四、流程 ③　分享給同事

`dev.mjs:700-708`。只寫一列 `members`，沒有外部資源。

**但有一個要注意的語意**：名單是**公司層級**的，不是單套系統。
加一個人進來，他會看到你們公司底下**所有**的系統。UI 上已經寫了這句話，
但它是產品決策，未來要做「只分享一套」得改資料模型。

---

## 五、流程 ④　部署到 Vercel

> **這是唯一產生「公開、免登入網址」的路徑，也是所有資安問題的集中處。**

### 觸發路徑

```
workspace.html 交付分頁的「部署到 Vercel」
  → POST /api/me/instances/<id>/vercel
  → dev.mjs:712-721   驗 owner 身分，然後 execFileSync（同步等，最多 15 分鐘）
  → tools/instance-deploy-vercel.mjs --instance=<id>
```

### 逐步做了什麼

| 步驟 | 程式 | 建立的東西 | 在誰的帳上 |
|---|---|---|---|
| 1 | `instance-deploy-vercel.mjs:118` | **第二個資料庫 `jv_<id>_pub`**，`CREATE TABLE LIKE` ＋ `INSERT SELECT` 整份複製 | 我們的 MySQL |
| 2 | `nextjs-bundle.mjs` | 把單檔 HTML 組成一個真的 Next.js 專案（App Router、`/api/t/[table]` 真路由） | 暫存目錄，用完即刪 |
| 3 | `:159` | **Vercel 專案**（`vercel link --project <name>`） | **Vercel 公司帳號** |
| 4 | `:167-175` | **production 環境變數**：`MYSQL_HOST/PORT/USER/PASSWORD` ＋ `MYSQL_DB=<pub>` | **Vercel 公司帳號** |
| 5 | `:181` | 部署（`vercel deploy --prod`） | Vercel |
| 6 | `:194` | 關掉 Vercel 的 SSO 保護（不關的話連擁有者自己都會被 302 到 vercel.com） | Vercel |
| 7 | `:219` | `events` 一列 `instance.vercel_deployed`，`detail_json` 裡才有 `url` 與 `project` | 我們的 MySQL |

### ⚠️ Vercel 專案名會跨客戶撞名

`instance-deploy-vercel.mjs:105`：

```js
const project = `jv-${inst.repo_name.replace(/^jvision-/, "").replace(/[^a-z0-9-]/g, "-")}`
  .slice(0, 52).replace(/-+$/, "");
```

專案名是從 **`repo_name`** 推的，**不含 instance id**。

後果：
- 兩個客戶都複製了同一套 demo → **同一個 Vercel 專案名**
- B 客戶按部署 → **覆蓋掉 A 客戶的線上站**（連 env 裡的 `MYSQL_DB` 都被改成 B 的）
- 未來加上刪除功能 → 刪 A 會**刪掉 B 正在用的站**

對照組：GitHub 交付的 repo 名是 `<repo>-<id 後 6 碼>`（`instance-deliver.mjs:151`），
**有帶 id**，所以不會撞。Vercel 這條是漏掉了。

### 資料的流向

```
jv_<id>（客戶真實資料）
   │  CREATE TABLE LIKE + INSERT SELECT   ← 單向，每次部署整份重來
   ↓
jv_<id>_pub  ←─ Vercel 上的公開站讀寫這一份
```

單向是對的（公開版寫入碰不到真資料）。
**但內容就是客戶的真實業務資料**，只是換了個 schema——公開讀取等於資料揭露。
詳見[問題 3](#3-公開的是真實資料的複本)。

---

## 六、流程 ⑤　交付到 GitHub

### ⚠️ 你的理解要修正：repo **不是**建在客戶自己的 GitHub

`instance-deliver.mjs:40`：

```js
const OWNER = args.owner || process.env.GITHUB_DELIVER_OWNER || "JVision-pj";
```

repo 建在 **`JVision-pj`——我們自己的 GitHub 組織**（`:202` 打的是 `/orgs/<OWNER>/repos`），
預設 **private**（`:35`，要公開得明講 `--public`，這個預設是對的）。

**所以工作台上「把這套系統推成一個你自己的 repo」這句文案是誤導的**——
客戶對那個 repo 沒有任何權限，除非有人手動把他加進組織。要嘛改文案，
要嘛做成「填入你的 GitHub 帳號 → 用 `--owner` 推到你那邊 ／ 建完自動邀請你當 collaborator」。

### 交付的內容

不是原始碼快照，是**可以自己跑起來的東西**：畫面、後端、資料表定義、
Docker 設定、CI。客戶 `docker compose up -d` 就能用，我們的主機關掉也不影響他。

**沒有推任何密碼上去**——`tools/templates/deliver/` 的樣板連線資訊全走環境變數，
README 教他 `cp .env.example .env` 自己填。這一點做得對。

| 建立的東西 | 在誰的帳上 | 可回收？ |
|---|---|---|
| GitHub repo `JVision-pj/<repo>-<id6>` | **我們的 GitHub 組織** | ❌ 沒有 |
| `instances.repo_url` 欄位 | 我們的 MySQL | ✅ 跟著實例走 |

---

## 七、資源總表：一個實例最多散落在 11 個地方

| # | 資源 | 何時建立 | 在誰的帳上 | 現有回收程式 |
|---|---|---|---|---|
| 1 | `instances` 一列 | 模板複製 | 我們 MySQL | ✅ `destroyInstance()` |
| 2 | 資料庫 `jv_<id>` | 模板複製 | 我們 MySQL | ✅ `destroyInstance()` |
| 3 | 檔案 `var/instances/<id>/` | 模板複製 | 我們磁碟 | ❌ |
| 4 | └ `uploads/` 客戶上傳的檔 | 使用中 | 我們磁碟 | ❌ **唯一一份** |
| 5 | **Cloudflare DNS 記錄** | 模板複製 | **Cloudflare** | ❌ |
| 6 | `orders` 一列 | 模板複製 | 我們 MySQL | ❌ |
| 7 | `events` 多列 | 全程 | 我們 MySQL | ❌ |
| 8 | `chat_sessions` / `chat_messages` | 使用中 | 我們 MySQL | ❌ **含使用者輸入** |
| 9 | 資料庫 `jv_<id>_pub` | 部署 Vercel | 我們 MySQL | ❌ |
| 10 | **Vercel 專案 ＋ production env（含 DB 密碼）** | 部署 Vercel | **Vercel** | ❌ |
| 11 | **GitHub repo** | 交付 GitHub | **我們的 GitHub 組織** | ❌ |

**現況：`destroyInstance()` 只處理了 11 項裡的 2 項。**

```js
// tools/lib/control-db.mjs:576
export async function destroyInstance(id) {
  const inst = await getInstance(id);
  if (!inst) return false;
  await dropDatabase(inst.db_name);            // ← 第 2 項
  await q("DELETE FROM instances WHERE id = ?", [id]);   // ← 第 1 項
  return true;
}
```

它原本只是給 `instance-verify.mjs` 清自己造的測試實例用的（`:187`），
**不是為使用者的「刪除」設計的**。今天照原樣接上去，結果會是：

> 主資料庫沒了，但 **Vercel 上那個站還活著、還對外公開、env 裡還握著完整的
> MySQL 帳密**，而且它讀的 `jv_<id>_pub` 也還在。DNS 記錄、GitHub repo、
> 客戶上傳的檔案、對話紀錄全部留著。

**這比不做刪除更糟。**

---

## 八、資源規劃：MySQL／Cloudflare／Vercel／GitHub

### MySQL

現況（全部住在 `sharemysql.theaken.com` 同一台）：

```
sharemysql.theaken.com
├── db_Survey            許願池（.env 的 MYSQL_*）
├── jv_demo_case         控制面：customers / members / orders / instances /
│                        events / quotas / chat_sessions / chat_messages / profiles
├── jv_<id> × 25         每個實例的業務資料
└── jv_<id>_pub × N      部署到 Vercel 的公開副本
```

每複製一套系統 **+1 個資料庫**；每部署一次 **再 +1**。這個成長是線性的，
共用主機遲早會撞到 schema 數或連線數上限。

**規劃建議**：

| 何時 | 做什麼 | 程式碼要改什麼 |
|---|---|---|
| 現在 | 建 `jv_pub` 專用帳號，只授權 `jv_%_pub` | `instance-deploy-vercel.mjs` 那 5 行 |
| 現在 | 許願池 `db_Survey` 搬走 | **只改 `MYSQL_HOST`**，零程式改動 |
| 定案時 | `JV_MYSQL_*` 整組搬到專用主機 | 改變數 ＋ 資料遷移，零程式改動 |
| 徹底隔離 | 控制面與實例再拆成兩台 | 要改 `tools/lib/mysql.mjs`——它現在只有一個連線池 |

`.env` 本來就是 `MYSQL_*` 與 `JV_MYSQL_*` 兩組獨立變數，前兩項幾乎是免費的。

### Cloudflare

- tunnel 只有一條（`1909ee29-...`），ingress 是萬用規則，**新增客戶不必動它**
- 每個實例一筆 DNS 記錄，目前 25 筆，**零回收**
- `~/.cloudflared/jvdemo.yml` 裡還留著舊的第四層命名 `*.c.jvdemo.jvision-ai.com`，
  註解說「等全部搬完再拿掉」——這是一筆待清的技術債
- 只能三層（`c-xxx.jvision-ai.com`），因為免費 Universal SSL 只簽一層萬用，
  四層沒憑證、瀏覽器 TLS 直接握手失敗

**規劃建議**：把 `routeDns()` 的反向操作補起來（`cloudflared tunnel route dns --overwrite-dns`
沒有刪除子命令，要走 Cloudflare API 的 `DELETE /zones/<zone>/dns_records/<id>`），
並在 `instances` 記下 DNS record id，否則刪除時要靠 host 反查。

### Vercel

- 一個公司帳號，專案名目前**會跨客戶撞名**（見上）
- 每個專案的 production env 裡有一份完整的 MySQL 憑證
- 部署是同步執行、最多等 15 分鐘（`dev.mjs:715`），一次只能跑一個

**規劃建議**：
1. 專案名改成 `jv-<repo>-<id 後 6 碼>`（跟 GitHub 一致）
2. `vercel_project` / `vercel_url` 存進 `instances` 表——現在只躺在 `events.detail_json` 裡，
   刪除時撈不到
3. 換成部署專用的 MySQL 帳號

### GitHub

- repo 建在 `JVision-pj` 組織下，private，名稱含 instance id（**不會撞名**）
- 沒有推任何密碼
- **但客戶對它沒有權限**——文案要改，或要做真正的「推到客戶自己的帳號」

---

## 九、待處理問題

排序依「不修的後果」而不是「修起來多快」。

### 1. 公開部署的憑證可以觸及整個平台

**嚴重度：最高。這是唯一一項「單點失守會波及所有客戶」的。**

`tools/instance-deploy-vercel.mjs:167-171`：

```js
["MYSQL_HOST", env("MYSQL_HOST")], ["MYSQL_PORT", env("MYSQL_PORT")],
["MYSQL_USER", env("MYSQL_USER")], ["MYSQL_PASSWORD", env("MYSQL_PASSWORD")],
["MYSQL_DB", pubDb],
```

帳號密碼是從 `.env` **原封不動**送過去的，只有「預設資料庫」被縮小到 `<實例>_pub`。

問題在於 **MySQL 的權限綁在帳號上，不是綁在預設資料庫上**。同一個帳號連進去之後
`USE` 別的資料庫是擋不住的。而且那組帳號確定權限很大——部署流程自己會呼叫
`createDatabase()` 建立 `_pub` 資料庫，也就是它有 `CREATE DATABASE`。

實際核對過資料庫的使用者清單，非系統帳號只有 `A999@%` 與 `aken@%`，
而 `.env` 裡 `MYSQL_*`（許願池）與 `JV_MYSQL_*`（控制面＋所有實例）**用的都是 A999**。
沒有任何分權。

**結論**：現在有一個公開、無身分驗證的 Vercel 應用，持有可以讀寫
`jv_demo_case`（客戶、成員、訂單、對話）與每一個 `jv_<實例>` 的憑證。

#### 怎麼修

```sql
CREATE USER 'jv_pub'@'%' IDENTIFIED BY '<夠長的隨機密碼>';
GRANT SELECT, INSERT, UPDATE, DELETE ON `jv\_%\_pub`.* TO 'jv_pub'@'%';
```

然後在 `.env` 加 `PUB_MYSQL_USER` / `PUB_MYSQL_PASSWORD`，把那五行改成讀這一組。
注意 `mirrorDatabase()` 需要 `CREATE DATABASE`，所以**建庫仍用管理帳號、部署出去的用 `jv_pub`**。

**成本**：約半小時，零應用程式邏輯改動。
**擋掉的**：把「平台級外洩」降成「單一 demo 外洩」。

### 2. 公開網址可以刪光資料，沒有任何身分驗證

`tools/lib/nextjs-bundle.mjs` 產生的 API：

| 端點 | 行號 | 作用 |
|---|---|---|
| `GET /api/t/<表>` | 199 | 讀任何一張表 |
| `POST /api/t/<表>` | 212 | 新增 |
| `PATCH /api/t/<表>/<id>` | 230 | 修改 |
| `DELETE /api/t/<表>/<id>` | 245 | 刪除 |

四個都沒有身分檢查，也沒有限流——整份檔案裡沒有出現過 `authorization`。
任何人拿到那個網址，就能把整套系統的資料刪光；而那是客戶要拿去給人看的東西。

#### 怎麼修（兩選一）

- **公開版唯讀**：`nextjs-bundle.mjs` 不產生 `POST`/`PATCH`/`DELETE` 三個 handler。
  要給人看的 demo，多半不需要對方能寫。
- **寫入端加 token**：部署時產生一組，寫進環境變數，前端帶在標頭上。

另外建議加上限流，避免有人反覆呼叫把資料庫打滿。

### 3. 公開的是真實資料的複本

`instance-deploy-vercel.mjs` 的註解寫著「公開的那份完全能用，但碰不到客戶真正的資料」——
這句話**只保護了寫入方向**。`mirrorDatabase()`（`:46-58`）是
`CREATE TABLE LIKE` ＋ `INSERT SELECT` 整份複製，**內容就是客戶的真實業務資料**
（名單、訂單、報價），只是放在另一個 schema。公開讀取等於資料揭露。

#### 怎麼修

在按下部署之前明確告訴客戶「這會把目前的資料複製到一個公開網址」，
並提供「只帶結構不帶資料」的選項（`CREATE TABLE LIKE` 之後不做 `INSERT SELECT`）。

這是產品決定，不只是技術決定。

### 4. Vercel 專案名跨客戶撞名

見[第五節](#-vercel-專案名會跨客戶撞名)。

**現在的後果**：B 客戶部署會覆蓋 A 客戶的線上站。
**加上刪除之後的後果**：刪 A 會刪掉 B 正在用的站。

#### 怎麼修

`instance-deploy-vercel.mjs:105` 改成含 instance id：

```js
const project = `jv-${inst.repo_name.replace(/^jvision-/, "")}-${inst.id.slice(-6)}`
  .replace(/[^a-z0-9-]/g, "-").slice(0, 52).replace(/-+$/, "");
```

**注意**：改了之後既有的部署會變成孤兒專案（舊名字的專案還在 Vercel 上、還公開、
env 還有密碼）。改名的同時要人工去 Vercel 後台清掉舊的，或寫一支一次性的搬遷腳本。

**成本**：程式一行，但要配一次人工清理。**必須在做刪除功能之前完成。**

### 5. 沒有任何外部資源的回收路徑

> 狀態：**部分改善**（2026-09-03）

見[第七節](#七資源總表一個實例最多散落在-11-個地方)。11 項資源只有 2 項有回收程式。

**這次多了一項工具、但還沒接上**：`lib/instance-dns.mjs` 現在有 `unpublish()`，
所以「子網域」這一項有能力回收了——但 `destroyInstance()` 沒有呼叫它，
它目前只做兩件事：

```js
await dropDatabase(inst.db_name);
await q("DELETE FROM instances WHERE id = ?", [id]);
```

也就是說刪掉一個實例之後，DNS 記錄、`var/instances/<id>/` 目錄、Vercel 專案、
`_pub` 資料庫、GitHub repo 全部留在原地。做刪除功能時 `destroyInstance()` 至少
要補上 DNS 與實例目錄——那兩項現在都有現成的做法，沒有理由留著。

### 6. 「交付到 GitHub」的文案與實際行為不符

> 狀態：**文案已修，定位待老闆確認**（2026-09-03）

見[第六節](#-你的理解要修正repo-不是建在客戶自己的-github)。
repo 建在我們的組織下，客戶沒有權限，而 UI 原本說「你自己的 repo」。

**文案已改**：交付卡現在寫「把這套系統推成一個 repo」，不再聲稱是他自己的。

**定位變成刻意的，不再是 bug**：老闆可能想收開發者的維運費——那個商業模式下
「開發者把 app 佈署到我們本機、GitHub 也放我們組織」是合理的，不是缺陷。
所以這一項從「要修的錯」變成「要確認的方向」。

真正要做的是**讓它可選**：綁定做成每個客戶自己的設定，沒綁就用我們組織、
綁了就推到他自己名下。兩種模式並存，不必現在選邊。綁定機制有兩條路：
客戶貼一把 fine-grained PAT（今天就能上線，不用註冊任何東西），或註冊一個
GitHub OAuth App（體驗最好，但要先在 GitHub 上申請 client_id／secret）。

**歷史資料的部分沒有包袱**：核對過整個組織 2073 個 repo，其中 2067 個是
`jvision-*` 型錄鏡像、5 個是我們自己的專案（`InnovationOS`、`panjit-ai-talent-hub`、
`panjit-crm`、`notary-system`、`spring-renta`），**客戶交付產物只有 1 個**
（`crossborder-localization-compliance-95c081`，擁有者是 aken1023 自己）。
沒有任何外部客戶的程式碼卡在我們組織裡。

### 7. 所有資料庫住在同一台伺服器

見[第八節](#mysql)。加上問題 1，等於「公開應用的憑證」與「公司其他重要資料」在同一台機器上。

### 8. `c-xxx.jvision-ai.com` 子網域從來沒有能用過

> 狀態：**✅ 已修**（2026-09-03）

> **已解決。** 三個阻擋都修掉了，實測未登入打子網域 3 跳直達 Google 登入頁，
> 持票換得 cookie 後拿到 HTTP 200 的系統頁。修法與下面「怎麼修」的規劃有一處
> 不同：**沒有**把 cookie 放寬成 `Domain=.jvision-ai.com`。那會把主站身分送到
> 每一個客戶子網域，而客戶系統的 HTML 是助理改得動的——省下的只是幾十行，
> 換掉的是整條信任邊界。改用 `visitor-auth.mjs` 的**交接票**：登入一律在主站辦，
> 成功後簽一張 90 秒、綁死目標主機名的票送回子網域，子網域驗票後發**自己的**
> host-only cookie。兩邊各管各的 cookie，誰也拿不到誰的。
>
> 同時子網域改成**按下佈署才建 DNS**（`lib/instance-dns.mjs`），不再是開通當下
> 就存在——使用者沒按，就不該有任何對外的網址。
>
> 另外清掉了 Cloudflare 上 26 筆沒人用得到的記錄（23 筆從未佈署 + 3 筆查無實例
> 的殘骸），保留按過佈署的 2 筆；刪除前的完整記錄留在 `var/backup/`。

<details><summary>當初的診斷（保留備查）</summary>


Cloudflare 上有 25 筆這樣的 DNS 記錄，**每一筆都連不進去**。
實測 `c-petty09190-crossborder-localiza.jvision-ai.com`：DNS 解析正常、
HTTPS 有回應、gateway 也正確認出它是實例——然後就卡在登入，永遠進不去。

三個獨立的阻擋疊在一起，缺一個都還是不通：

#### ① 無限重導迴圈

`dev.mjs:274-279` 把「客戶子網域」判在**所有主站路由之前**：

```js
const inst = await instanceForHost(req.headers.host);
if (inst) { ...; return serveInstance(req, res, inst.id, req.url || "/", ...); }
```

於是在 `c-xxx` 這個 host 上，`/api/visitor/google/start`
**永遠到不了它自己的處理器**（那在 `dev.mjs:303`），而是被 `serveInstance` 接走；
`serveInstance` 看到沒有身分，就 302 到 `/api/visitor/google/start?next=<現在這個網址>`
——同一個 host，於是無限繞。

實測：

```
$ curl -i https://c-petty09190-crossborder-localiza.jvision-ai.com/api/visitor/google/start?next=%2F
HTTP/2 302
location: /api/visitor/google/start?next=%2Fapi%2Fvisitor%2Fgoogle%2Fstart%3Fnext%3D%252F
```

那個 `next` 一層一層疊上去，就是迴圈的證據。

#### ② cookie 是 host-only

`visitor-auth.mjs:64` 設 cookie 時**沒有 `Domain=`**：

```js
appendCookie(res, `${COOKIE}=${payload}.${sign(payload)}; Path=/; HttpOnly...; SameSite=Lax; ...`);
```

沒有 `Domain` 的 cookie 是 host-only。所以就算繞過 ①、跑到主站登入完，
`jvdemo.jvision-ai.com` 拿到的身分**不會**被送到 `c-xxx.jvision-ai.com`。

#### ③ Google OAuth 的 redirect_uri 不能用萬用字元

`google-auth.mjs:38-45` 的 `redirectUri(req)` 是從**進來的 Host 標頭**組的：

```js
export const redirectUri = (req) => `${originOf(req)}/api/admin/google/callback`;
```

所以每個子網域會產生自己的一組 redirect_uri，而 Google 要求每一組都要
**事先登記在 OAuth 用戶端裡，且不接受萬用字元**。25 個子網域就要登記 25 次，
每開通一個客戶再登記一次——這條路本質上走不通。

#### 怎麼修（如果決定要留 B）

要同時解掉三個：

1. **登入一律回主站**：`serveInstance` 沒身分時，改導到
   `https://jvdemo.jvision-ai.com/api/visitor/google/start?next=<原子網域的完整網址>`。
   這樣 Google 只需要登記**一組** redirect_uri（③ 解掉），
   而且登入路由不再被實例路由吃掉（① 解掉）。
2. **cookie 加 `Domain=.jvision-ai.com`**（② 解掉）。
3. **`next` 的白名單要改**：現在 `dev.mjs:886` 是 `location: rec.next || "/"` 直接用，
   要改成「只允許 `*.jvision-ai.com`」——順便修掉[問題 9](#9-登入的-next-參數是開放重導)。

⚠️ **第 2 步的代價要認**：`Domain=.jvision-ai.com` 表示這個 cookie 會被送到
`jvision-ai.com` 底下的**每一個**子網域，包含未來任何架在同網域的服務。
現在的 host-only 是比較嚴的設定。要放寬之前，先確認 `.jvision-ai.com` 上
不會有第三方或低信任度的服務。

**成本**：三處小改動，但要一起改才有效果，而且要重新想清楚 cookie 的信任邊界。

</details>

### 9. 登入的 `next` 參數是開放重導

> 狀態：**✅ 已修**（2026-09-03）

> **已解決。** 加了 `safeNext()`：只認站內的相對路徑（`//` 開頭視為外部，擋掉），
> 以及**資料庫裡真的存在的**實例子網域——查資料庫而不是比對字串樣式，因為
> `c-任何東西.jvision-ai.com` 都符合樣式，但只有真的開通過的才算數。其餘一律
> 回首頁。後台那條只收站內路徑（後台身分不跨子網域）。
> 實測 `to=https://evil.com` 與 `to=//evil.com` 都導不出去。


`dev.mjs:304` 收下 `next` 完全不驗證，`dev.mjs:886` 登入成功後直接拿它當 302 目標：

```js
const next = new URL(req.url, "http://x").searchParams.get("next") || "/";   // :304
...
res.writeHead(302, { location: rec.next || "/" });                            // :886
```

所以 `https://jvdemo.jvision-ai.com/api/visitor/google/start?next=https://evil.com`
會在使用者**成功用 Google 登入之後**把他送去 evil.com。
釣魚會用到這種連結：網址開頭是真的、憑證是真的、登入畫面是真的。

`welcome.html` 前端有擋（`if (!/^\/(?!\/)/.test(next)) next = "/"`），
但那只是前端；直接打這個 API 就繞過去了。

#### 怎麼修

在 `dev.mjs:304` 收下 next 時就驗：相對路徑（`/` 開頭但不是 `//`）放行，
絕對網址只允許 `*.jvision-ai.com`。一個函式、五行。

### 附帶：`.env` 裡該輪替的憑證

以下曾出現在截圖中，建議直接換發：

- `VERCEL_TOKEN`
- `GITHUB_TOKEN`
- `OLLAMA_API_KEY`
- `MYSQL_PASSWORD` / `JV_MYSQL_PASSWORD`（同一組 A999，現值只有四位數）

`.env` 不在版控裡，換完不需要改任何程式碼。

---

### 10. 交付用的 GitHub token 是個人帳號權杖

**這次新發現。** `instance-deliver.mjs:39` 讀的 `GITHUB_TOKEN` 實測是
**`aken1023` 這個個人帳號**的權杖（`GET /user` 回 `login: aken1023, type: User`），
不是機器人帳號，而且對 `JVision-pj` 組織有寫入權。

那個組織裡有 **2065 個公開 repo ＋ 8 個私有 repo**，私有的包含
`panjit-crm`、`spring-renta`、`notary-system` 這些真正的客戶專案。

也就是說：**客戶在工作台按一下「交付」，背後跑的是一把能碰到你們所有私有
專案的憑證。** 就算之後 repo 改建到客戶自己名下，這件事還是成立——執行交付的
那把鑰匙不該這麼大。

順帶記下同一把 token 缺的兩個權限（這是為什麼「開 PR」一直失敗）：

| 權限 | 缺了會怎樣 |
|---|---|
| `pull_requests`（寫） | `POST /pulls` 回 403，PR 開不出來 |
| `workflow` | 推不了 `.github/workflows/`，交付出去的 repo 沒有 CI |

#### 怎麼修

1. 開一個專用的機器人帳號（或 GitHub App），只邀請它進**交付要用的那個範圍**，
   不要給整個組織。
2. 用 fine-grained token，勾 `Contents` 讀寫、`Administration` 寫（要建 repo）、
   `Pull requests` 寫、`Workflows` 寫。
3. `.env` 換成那一把，把 `aken1023` 的個人權杖從產線環境撤掉。

**成本**：GitHub 後台十幾分鐘，程式零改動（`GITHUB_TOKEN` 的讀法不變）。
**擋掉的**：把「客戶按一顆按鈕會動用公司最大權限」降成「只能動它自己的 repo」。

## 十、刪除功能該怎麼做

### 分成兩段，不要一步到位

#### 第一段：封存（現在就能做，可逆，不碰任何外部資源）

**基礎建設已經全部就緒，只差有人去設定那個狀態：**

| 已經有的 | 位置 |
|---|---|
| `instances.state` 欄位 | `control-db.mjs:65` |
| `instances.archived_at` 欄位 | `control-db.mjs:69` |
| 所有查詢都已經 `WHERE state <> 'archived'` | `control-db.mjs:478, 541, 563, 573` |
| gateway 認得並回 410 | `dev.mjs:276` |
| app-server 認得並回 410 | `app-server.mjs:101` |

**要寫的只有**：一支 `POST /api/me/instances/<id>/archive`（驗 owner）
呼叫 `setInstanceState(id, "archived", { archived_at: now })`，
以及工作台上的一顆按鈕 ＋ 一個確認對話框。

效果：從「我的專案」消失、對外網址回 410、資料一個位元組都沒動、隨時可以還原。

**這一段不依賴部署流程定案，可以馬上做。**

#### 第二段：硬回收（要等部署定案）

真正的 `DELETE` 要按這個順序拆（**由外而內，先斷對外暴露再刪資料**）：

```
1. Vercel 專案      ← 先斷。最公開、風險最高、還握著密碼
   （前提：專案名要能唯一對應到這個實例 → 問題 4 必須先修）
2. Cloudflare DNS   ← 斷對外解析
3. jv_<id>_pub      ← DROP，公開副本
4. GitHub repo      ← 要不要刪是產品決定（客戶可能還要）
5. var/instances/<id>/  ← 含 uploads/，客戶上傳的檔案是唯一一份，
                          建議先打包成一個 zip 交給客戶再刪
6. chat_sessions / chat_messages / events   ← 級聯刪除
7. jv_<id>          ← DROP，主資料庫
8. instances / orders 那幾列
```

**每一步都要冪等**，中間失敗可以重跑——這是整個 codebase 一致的原則
（`instance-provision.mjs` 開頭就寫了「共用主機的連線會斷，重跑比回滾實際」）。

#### 解鎖第二段所需的最小改動

| # | 改什麼 | 為什麼是刪除的前置條件 |
|---|---|---|
| 1 | Vercel 專案名加 instance id | 不改的話刪 A 會刪到 B |
| 2 | `instances` 加 `vercel_project` / `vercel_url` 欄位 | 現在只在 `events.detail_json` 裡，撈不到 |
| 3 | `instances` 加 `dns_record_id` 欄位 | 不然要靠 host 去 Cloudflare API 反查 |

三件都是小改動，但**都必須在刪除功能上線前完成**。

---

## 十一、建議順序

> 2026-09-03 重寫。原本的順序建立在「方向還沒定」這個前提上，而那個前提已經
> 不成立了——子網域修好了、佈署按鈕走 Cloudflare、Vercel 藏起來。剩下的項目
> 也因此重排。

### 第 0 順位：把還活著的曝險關掉

這一項排在所有東西之前，因為它**現在正在發生**，而且不需要任何決策：

| 步驟 | 做什麼 |
|---|---|
| 1 | 刪掉 `jv-inventory` 與 `jv-construction-insurance-reconcili` 兩個 Vercel 專案（含環境變數裡的 MySQL 憑證） |
| 2 | `DROP DATABASE` 那 3 個 `_pub`（`jv_imtidi0ezdffaa5_pub`、`jv_imtiljt16691b85_pub`、`jv_imtiq1fi19140c2_pub`） |
| 3 | 順手確認 `jv-bid-bond-tracking` 這個 Vercel 專案（已經 404，但專案還在，env 也還在） |

按鈕已經藏起來了，所以刪掉不會影響任何人——**沒有任何使用者現在走得到那條路**。
反過來說，不刪就等於留著兩個公開、可寫、持有平台級憑證的端點。

成本：Vercel 後台十分鐘 ＋ 三行 SQL。

### 第 1 順位：兩把權限過大的鑰匙

兩件事同一個性質：一顆按鈕背後動用了遠超過它需要的權限。

| 順位 | 做什麼 | 為什麼 | 成本 |
|---|---|---|---|
| 1a | [問題 10](#10-交付用的-github-token-是個人帳號權杖)：GitHub 換專用機器人帳號／App，順便補 `pull_requests` 與 `workflow` | 現在客戶按「交付」跑的是能碰公司所有私有專案的個人權杖 | 十幾分鐘，程式零改動 |
| 1b | [問題 1](#1-公開部署的憑證可以觸及整個平台)：`jv_pub` 專用 MySQL 帳號 | 只要 Vercel 那條路還留著程式碼，哪天再打開就又是平台級曝險 | 半小時 |

1b 如果第 0 順位已經把 Vercel 的東西清乾淨，可以降一級——但**不要跳過**：
程式碼還在，隨時可能有人把 `hidden` 拿掉。

### 第 2 順位：使用者現在就要的功能

| 順位 | 做什麼 | 現況 | 成本 |
|---|---|---|---|
| 2a | **封存功能** | 只有 `archived_at` 欄位與 `state='archived'` 的判斷，沒有 API 也沒有畫面 | 半天 |
| 2b | **GitHub 綁定做成可選** | 見問題 6。等老闆確認維運費那個方向 | PAT 半天／OAuth 一天 |

### 第 3 順位：刪除功能

前提比原本少了很多——B 那條路已經有回收函式、Vercel 如果按第 0 順位清掉就不必
處理。但 `destroyInstance()` 還是要補：

- 呼叫 `dns.unpublish(inst.host)`（函式已經有了，只是沒人叫）
- `rm -rf var/instances/<id>/`
- GitHub repo **不要自動刪**——那是客戶的東西，從我們這邊按一顆按鈕毀掉他的
  程式碼，誤按的代價太大。提示他自己去 GitHub 刪比較合理。

如果 Vercel 那條路將來要重新打開，[問題 4](#4-vercel-專案名跨客戶撞名)（專案名撞名）
必須先修——不修的話「刪 A 客戶」會刪掉 B 客戶正在用的站。

### 第 4 順位：還沒動、但也還不急

- [問題 2](#2-公開網址可以刪光資料沒有任何身分驗證)（公開版可寫可刪）與
  [問題 3](#3-公開的是真實資料的複本)（公開真資料）：只在 Vercel 那條路重新打開時才成立。
  真要重開，這兩項是前置條件，不是選項。
- [問題 7](#7-所有資料庫住在同一台伺服器)：等架構定案一起規劃。

### 不論走哪條

- **第 0 順位無條件先做。** 它不依賴任何決策，而且是唯一「現在正在漏」的一項。
- **硬刪除永遠排在最後。** 在對外路徑收斂完之前動它，等於在還不知道有幾個
  出口的房子裡拆牆。
