...
- **Continuous deployment:** A local authenticated relay can fast-forward a clean `feat/homepage-impact` checkout, transfer a Git bundle over a dedicated SSH key, validate each candidate in an isolated server worktree, publish it only after runtime checks and roll back automatically when the release health check fails.
