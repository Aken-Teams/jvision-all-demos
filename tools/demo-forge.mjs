...
## 唯一的例外：records.columns 是佔位，你必須換掉
規格書裡的 \`records.columns\`（編號／項目／負責人／期限／階段）是**樣板佔位**，
不是這套系統真正的欄位。除了它，其餘規格一律照做。

表格的 \`<th>\` 必須是這個領域的人真的會講的名字。客戶按「模板複製」買走的
就是畫面上那張表——欄位叫「項目」的話，它跟任何一套通用待辦清單沒有差別。

以《開挖支撐巡查交班台》為例：
  ✗ 編號、項目、負責人、期限、階段
  ✓ 開挖區段、支撐型式、巡查人、交班時間、量測位移、異常狀態
以《混凝土澆置批次管理》為例：
  ✗ 編號、項目、負責人、期限、階段
  ✓ 澆置編號、配比強度、車次、坍度、澆置區域、試體編號

規則：
- 每一張表至少 5 欄，欄名要能讓這一行的人一看就知道是什麼。
- 不可以出現「編號、項目、負責人、期限、階段」這一整組。單獨用「負責人」
  或「狀態」是可以的，整組照抄不行。
- 資料列的內容要跟著換成該領域真實會出現的值（不要 D+1／處理中 這種佔位）。
