 ...
 *
 * 隔離用**表前綴**：每個實例的表都叫 i_<實例編號>_<表名>。共用一個資料庫時，
 * 靠 WHERE tenant_id 隔離只要漏寫一次就是跨租戶外洩；把租戶編號放進表名，
 * 漏寫條件也撈不到別人的資料——這是共用資料庫下最接近「一個客戶一個檔」的做法。
 *
 * 表名一律經過 ident() 白名單化才拼進 SQL（表名不能用參數綁定）。
 */
import { q, one, tx, ident } from "./mysql.mjs";
...
const tableOf = (prefix, name) => ident(`${prefix}${name}`.replace(/`/g, "")).replace(/`/g, "");
const now = () => new Date().toISOString().slice(0, 19).replace("T", " ");

/**
 * 依 schema 建立這個實例的資料表。冪等，可以重跑。
 * @param {string} prefix instances.table_prefix，例如 i_imte1a2b3_
 * @param {object} schema content/schema/<repo>.json 的內容
 */
export async function createFromSchema(prefix, schema, { seed = true } = {}) {
  const meta = tableOf(prefix, "_meta");
  const colsT = tableOf(prefix, "_columns");
  const auditT = tableOf(prefix, "_audit");

  await q(`CREATE TABLE IF NOT EXISTS ${ident(meta)} (
    \`key\` VARCHAR(40) PRIMARY KEY, value TEXT) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
  await q(`CREATE TABLE IF NOT EXISTS ${ident(colsT)} (
  ...
  await q(`CREATE TABLE IF NOT EXISTS ${ident(auditT)} (
  ...
  for (const [k, v] of [["repo_name", schema.repoName], ["created_at", now()], ["schema_version", "1"]]) {
    await q(`INSERT INTO ${ident(meta)}(\`key\`,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)`, [k, v]);
  }

  for (const t of schema.tables) {
    const phys = tableOf(prefix, t.name);
    const defs = t.columns.map((c) => `${col(c.key)} ${SQL_TYPE[c.type] || "TEXT"}`).join(",\n      ");
    await q(`CREATE TABLE IF NOT EXISTS ${ident(phys)} (
    ...
    for (const [i, c] of t.columns.entries()) {
      await q(`INSERT INTO ${ident(colsT)}(table_name,\`key\`,label,type,ord) VALUES(?,?,?,?,?)
    ...
    if (seed && t.seed?.length) {
      const existing = await one(`SELECT COUNT(*) c FROM ${ident(phys)}`);
      if (!existing.c) {
        const keys = t.columns.map((c) => c.key);
        const cols = keys.map(col).join(",");
        const marks = keys.map(() => "?").join(",");
        for (const row of t.seed) {
          await q(`INSERT INTO ${ident(phys)} (${cols}, _created_at, _updated_at, _created_by)
...
export async function describe(prefix) {
  const colsT = ident(tableOf(prefix, "_columns"));
  const names = await q(`SELECT DISTINCT table_name FROM ${colsT} ORDER BY table_name`);
  const repo = await one(`SELECT value FROM ${ident(tableOf(prefix, "_meta"))} WHERE \`key\`='repo_name'`);
  const tables = [];
  for (const n of names) {
    const columns = await q(`SELECT \`key\`,label,type FROM ${colsT} WHERE table_name=? ORDER BY ord`, [n.table_name]);
    const c = await one(`SELECT COUNT(*) c FROM ${ident(tableOf(prefix, n.table_name))} WHERE _deleted_at IS NULL`);
...
async function assertTable(prefix, table) {
  const ok = await one(`SELECT 1 x FROM ${ident(tableOf(prefix, "_columns"))} WHERE table_name=? LIMIT 1`, [table]);
  if (!ok) throw Object.assign(new Error(`沒有這張表：${table}`), { status: 404 });
  return ident(tableOf(prefix, table));
}

const keysOf = async (prefix, table) =>
  (await q(`SELECT \`key\` FROM ${ident(tableOf(prefix, "_columns"))} WHERE table_name=? ORDER BY ord`, [table]))
    .map((r) => r.key);

export async function list(prefix, table, { limit = 50, offset = 0, q: search = "" } = {}) {
  const phys = await assertTable(prefix, table);
  const keys = await keysOf(prefix, table);
...
export async function create(prefix, table, values, actor) {
  const phys = await assertTable(prefix, table);
  const keys = await keysOf(prefix, table);
  return tx(async (cn) => {
    ...
    const id = r.insertId;
    await audit(cn, prefix, { actor, table, rowId: id, action: "create", after: values });
 ...
 */
export async function update(prefix, table, id, values, rev, actor) {
  const phys = await assertTable(prefix, table);
  const keys = await keysOf(prefix, table);
  const set = {};
  ...
  return tx(async (cn) => {
    ...
    await audit(cn, prefix, { actor, table, rowId: id, action: "update", before: before[0], after: set });
    const [rows] = await cn.execute(`SELECT _id, ${keys.map(col).join(",")}, rev FROM ${phys} WHERE _id=?`, [id]);
...
export async function remove(prefix, table, id, actor) {
  const phys = await assertTable(prefix, table);
  return tx(async (cn) => {
    const [before] = await cn.execute(`SELECT * FROM ${phys} WHERE _id=? AND _deleted_at IS NULL`, [id]);
    if (!before.length) return false;
    await cn.execute(`UPDATE ${phys} SET _deleted_at=?, _updated_at=?, rev=rev+1 WHERE _id=?`, [now(), now(), id]);
    await audit(cn, prefix, { actor, table, rowId: id, action: "delete", before: before[0] });
    return true;
  });
}

/** 客戶自己加欄位。ADD COLUMN 之後舊資料那一欄是 NULL，不會壞。 */
export async function addColumn(prefix, table, { key, label, type = "text" }, actor) {
  const phys = await assertTable(prefix, table);
  const colsT = ident(tableOf(prefix, "_columns"));
  col(key);
...
/** 實例整個移除（退款或封存後）。表前綴讓這件事很乾淨：刪掉該前綴的表就好。 */
export async function dropAll(prefix) {
  const rows = await q("SELECT table_name t FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name LIKE ?",
    [`${prefix}%`]);
  for (const r of rows) await q(`DROP TABLE IF EXISTS ${ident(r.t)}`);
  return rows.length;
}

async function audit(cn, prefix, { actor, table, rowId, action, before = null, after = null }) {
  await cn.execute(`INSERT INTO ${ident(tableOf(prefix, "_audit"))}(at,actor,table_name,row_id,action,before_json,after_json)
    VALUES(?,?,?,?,?,?,?)`,
