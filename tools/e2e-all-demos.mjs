...
async function inspect(page,project,index){
  ...
  page.on("response",response=>{if(response.url().startsWith(baseUrl)&&response.status()>=400)httpErrors.push(`${response.status()}: ${response.url()}`)});
  ...
  if(!navigationError){
    desktop=await page.evaluate(()=>{
      ...
      return {textLength:(document.body?.innerText||"").trim().length,title:document.title.trim(),heading:(document.querySelector("h1")?.textContent||"").trim(),interactiveCount:interactive.length,errorOverlay:Boolean(document.querySelector('[data-nextjs-dialog],nextjs-portal,.vite-error-overlay,#webpack-dev-server-client-overlay')),horizontalOverflow:Math.max(0,Math.round(overflow))};
    });

    let candidate=page.locator('button:not([disabled]):visible,select:not([disabled]):visible').first();
  ...
  if(desktop.horizontalOverflow>8)reasons.push(`desktop overflow ${desktop.horizontalOverflow}px`);
  if(mobile.horizontalOverflow>8)reasons.push(`mobile overflow ${mobile.horizontalOverflow}px`);
...
const rows=new Array(catalog.projects.length);
let cursor=0;
let completed=0;
