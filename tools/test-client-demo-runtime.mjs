  ...
  for (const project of samples) {
    ...
    const result = await page.evaluate(() => {
      ...
      return {
        ...
        variant: root?.classList.contains("jv-domain-demo") ? "domain" :
          root?.classList.contains("jv-oee-demo") ? "oee" : "generic",
        title: root?.querySelector("h2")?.textContent || "",
        records: root?.querySelectorAll(".jv-demo-record, .jv-domain-items article").length || 0,
        stages: root?.querySelectorAll(".jv-demo-stage, .jv-domain-flow button").length || 0,
        createButtons: root?.querySelectorAll(".jv-demo-create button, .jv-domain-create button[type=submit]").length || 0,
        guideVisible: Boolean(root?.querySelector(".jv-demo-guide:not([hidden]), .jv-domain-guide:not([hidden])")),
    ...
    if (result.variant === "domain") {
      interactionPassed = await page.evaluate(() => {
        const before = document.querySelector(".jv-domain-items article .jv-domain-status")?.textContent;
        const advance = document.querySelector(".jv-domain-items article [data-domain-advance]:not([disabled])");
        if (!before || !advance) return false;
        advance.click();
        const after = document.querySelector(".jv-domain-items article .jv-domain-status")?.textContent;
