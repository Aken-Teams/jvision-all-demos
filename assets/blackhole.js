 ...
 * 黑洞 hero 背景，原生 JS 版。
 *
 * 原始元件是 React + TypeScript（shadcn 結構）。這個專案沒有 React、沒有 TS、
 * 也沒有任何建置流程——20 個靜態 HTML 由 serve 直接送。為了一個背景導入
 * React + Vite + Tailwind 等於給整站加一條建置管線，而這個站的前提正好相反：
 * 每個檔案自足、打開就能跑。
 *
 * 所以移植而不是引入：著色器一字未改（那才是這個元件的價值），React 的部分
 * 只是 useEffect 生命週期，換成 initBlackHole(host, options) 回傳 dispose()。
 *
 * 畫的是把光按照黑洞彎曲光線的方式彎過的黑洞。每個像素射出一條光線，在彎曲
 * 時空裡反向走，直到穿過視界、飛向星空、或穿過吸積盤——常常穿三四次，因為
 * 光線會繞著洞轉回來。盤只畫一次；盤上方的光暈、下方的光暈、以及緊貼陰影的
 * 那圈細環，都是同一個盤被彎曲的光再看見一次。它們對得起來，是因為沒有人去
 * 對齊它們。
 *
 * 單位由視界決定：r = 1 是事件視界，光子球在 1.5，陰影半徑 2.6，盤從 3 開始
 * （這個幾何下氣體能維持的最內軌道）。這些數字不是調出來的，是從度規掉出來的。
 */
(function (global) {
  "use strict";
...
`;
  ...
  var DEFAULTS = {
    distance: 24, elevation: -5.5, azimuth: 0, orbitSpeed: 0, roll: -20, fov: 42,
    diskInner: 3, diskOuter: 15, diskThickness: 0.26, diskDensity: 1, brightness: 1,
    spinSpeed: 0.06, grain: 0.48, doppler: 0.35,
    ...
    focus: [0.72, 0.46], scrim: "none", scrimStrength: 0.9, paused: false,
  };

  /**
   * @param {HTMLElement} host 容器；canvas 會被塞進去並填滿它
   * @param {object} options 見 DEFAULTS
   * @returns {{dispose:function, set:function, ok:boolean}}
   */
  function initBlackHole(host, options) {
    var C = {};
    for (var k in DEFAULTS) C[k] = DEFAULTS[k];
    for (var k2 in (options || {})) if (options[k2] !== undefined) C[k2] = options[k2];
    ...
    /* 沒有 WebGL 就把 canvas 拿掉，留下黑底與上面的文字。context 掛掉的 canvas
       不會安靜地消失——它會畫成白色或破圖標記蓋在背景上，比什麼都不顯示更糟。 */
    function giveUp(why) {
      host.dataset.webgl = why;
      canvas.remove();
    }
    if (!gl) { giveUp("unsupported"); return { ok: false, dispose: function () {}, set: function () {} }; }
    ...
    var hdr = true, texType = gl.UNSIGNED_BYTE, internal = gl.RGBA;
    if (isGL2) {
      var ok = gl.getExtension("EXT_color_buffer_half_float") || gl.getExtension("EXT_color_buffer_float");
    ...
    var settled = 0, width = 0, height = 0, sceneW = 0, sceneH = 0;
    ...
    function resize() {
      ...
      var dpr = software ? 1 : Math.min(window.devicePixelRatio || 1, Math.max(1, C.maxDpr));
      var cssW = Math.max(1, Math.round(rect.width));
      var cssH = Math.max(1, Math.round(rect.height));
      var scale = software ? 0.34 : Math.min(1, Math.max(0.4, C.resolution));
    ...
    var clock = reduced ? 6 : 0, lastFrame = 0, running = true, visible = true, raf = 0;
    ...
    function render(t) {
      if (!sceneProg || !scene || !histA || !histB || !bloomA || !bloomB) return;

      var az = (C.azimuth + C.orbitSpeed * t) * RAD;
      var el = Math.max(-88, Math.min(88, C.elevation)) * RAD;
      ...
      var cr = Math.cos(C.roll * RAD), sr = Math.sin(C.roll * RAD);
      var RX = rx * cr + ux * sr, RY = ry * cr + uy * sr, RZ = rz * cr + uz * sr;
      var UX = -rx * sr + ux * cr, UY = -ry * sr + uy * cr, UZ = -rz * sr + uz * cr;

      var hot = hexToLinear(C.hotColor), mid = hexToLinear(C.midColor), cool = hexToLinear(C.coolColor);
      ...
      var hal = HALTON[settled % HALTON.length];
      ...
      pass(compProg, null);
      bind(shown.tex, 0); bind(bloomA.tex, 1);
    ...
    function settle(passes) { for (var i = 0; i < passes; i++) render(clock); }
    ...
    if (!build()) { giveUp("build-failed"); return { ok: false, dispose: function () {}, set: function () {} }; }
    resize();
    settle(reduced ? 16 : 1);
    if (!reduced) raf = requestAnimationFrame(tick);

    var ro = new ResizeObserver(function () { resize(); if (reduced || C.paused) settle(16); });
    ro.observe(host);
    ...
    function onLost(e) { e.preventDefault(); running = false; cancelAnimationFrame(raf); canvas.style.display = "none"; }
    function onRestored() {
      width = height = sceneW = sceneH = 0;
      if (!build()) { giveUp("lost"); return; }
      canvas.style.display = "";
      host.dataset.webgl = "";
      resize(); running = true; lastFrame = 0;
      settle(reduced ? 16 : 1);
      if (!reduced) raf = requestAnimationFrame(tick);
    }
    ...
    return {
      ok: true,
      set: function (next) {
        for (var key in (next || {})) if (next[key] !== undefined) C[key] = next[key];
        if (reduced || C.paused) settle(8);
      },
      dispose: function () {
        running = false;
        ...
        /* 刻意不呼叫 WEBGL_lose_context。看起來整潔，實際上是陷阱：context 一旦
           被弄丟，下一次 getContext 會拿回同一個死掉的 context，之後每個著色器
           都會以空白 log 編譯失敗。刪掉配置的東西就夠了，context 會跟著 canvas
           一起消失。 */
      },
    };
  }

  global.initBlackHole = initBlackHole;
})(window);
