...
const url = process.argv[2] || "http://127.0.0.1:3021";

const checks = [
  ["hasModules", "text=功能模組"],
  ["hasStudents", "text=學生與出勤"],
  ["hasCourses", "text=課程排班"],
  ["hasMessages", "text=聯絡簿與通知"],
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/education-care-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
