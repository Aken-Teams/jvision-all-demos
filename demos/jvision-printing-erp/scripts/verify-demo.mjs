...
const url = process.argv[2] || "http://127.0.0.1:3028";

const checks = [
  ["hasModules", "text=功能模組"],
  ["hasQuote", "text=估價與訂單轉單"],
  ["hasArtwork", "text=圖檔版模與合版"],
  ["hasWip", "text=製程移轉與託外追蹤"],
  ["hasDashboard", "text=成本與出貨儀表板"],
];
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/printing-erp-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
