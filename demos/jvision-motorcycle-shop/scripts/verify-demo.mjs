...
const checks = [
  ...
  ["hasCustomers", "text=客戶與車籍"],
  ["hasRepairs", "text=維修保養單"],
  ["hasInventory", "text=零件庫存與付款"],
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/motorcycle-shop-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
