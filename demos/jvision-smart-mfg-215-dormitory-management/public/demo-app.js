...
setupProjectPeopleModules({"id":1315,"name":"員工宿舍管理系統（Dormitory Management）","description":"管理工廠員工宿舍床位分配、住宿費用、門禁與生活公約，適用大量產線員工與外籍移工集中住宿場景。","modules":["宿舍總覽","床位配置","入住與報修","AI 宿舍調度"],"functions":["床位/房間分配與異動管理","住宿費用計算與代扣","宿舍門禁與訪客登記","公共設施報修申請","生活公約違規紀錄","退宿/清潔點交管理"],"workflows":["入住申請","床位分配","住宿費登記","門禁/訪客管理","違規/報修處理","退宿點交"],"pains":["大量入退宿床位調度混亂","住宿費代扣與薪資對帳耗時","違規事件（吸菸、聚眾）追蹤不易","消防安全與人數即時掌握困難"],"ai":["床位智慧分配最佳化","異常進出/群聚行為偵測預警"],"roles":["人資部","廠務行政","宿舍管理員","保全單位"],"metrics":["床位使用率","住宿費對帳正確率","違規事件處理時效","消防演練完成率"],"fields":["對象","期限","風險","負責人"]});
