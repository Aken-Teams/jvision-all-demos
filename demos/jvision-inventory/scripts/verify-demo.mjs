...
const forbidden = ["Odoo", "Odoo Inventory", "????", "undefined"];
...
await page.getByRole("button", { name: "執行掃碼異動" }).click();
await page.getByRole("button", { name: "生成 AI 庫存摘要" }).click();
await page.getByText("今日庫存總值").first().waitFor({ timeout: 10000 });
...
await page.goto(url, { waitUntil: "networkidle", timeout: 60000 });
const mobileHeading = await page.getByRole("heading", { name: "Jvision 智慧庫存與倉儲管理平台" }).isVisible();
