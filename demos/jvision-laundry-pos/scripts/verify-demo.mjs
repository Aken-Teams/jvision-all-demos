...
const url = process.argv[2] || "http://127.0.0.1:3025";

const checks = [
  ["hasModules", "text=功能模組"],
  ["hasCustomers", "text=客戶與收件"],
  ["hasOrders", "text=送洗衣物"],
  ["hasPayment", "text=付款與報表"],
  ["hasDashboard", "text=營業儀表板"],
];
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/laundry-pos-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
