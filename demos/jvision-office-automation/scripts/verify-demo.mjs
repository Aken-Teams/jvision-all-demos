...
const forbidden = ["O2OA", "翱途", "浙江兰德", "浙江蘭德", "????", "??"];
...
await page.getByRole("button", { name: "送出簽核單" }).click();
await page.getByRole("button", { name: "生成 AI 摘要" }).click();
await page.getByLabel("搜尋門戶內容").fill("資產");
await page.getByRole("button", { name: "開啟內容" }).first().click();
...
await page.goto(url, { waitUntil: "networkidle", timeout: 60000 });
const mobileHeading = await page.getByRole("heading", { name: "Jvision 企業數位協同辦公中樞" }).isVisible();
