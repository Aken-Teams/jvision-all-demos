...
const forbidden = ["創盛", "創盛國際", "the-creations", "????", "已改為", "相關字樣"];
...
await page.getByRole("link", { name: "開啟申報 Demo" }).click();
await page.getByRole("button", { name: "新增處方並試算" }).click();
await page.getByRole("button", { name: "標記可申報" }).first().click();
await page.getByRole("button", { name: "生成 AI 申報摘要" }).click();
await page.getByRole("button", { name: "列印藥袋與收據" }).click();
...
await page.goto(url, { waitUntil: "networkidle", timeout: 60000 });
const mobileHeading = await page.getByRole("heading", { name: "Jvision 藥局健保調劑申報系統" }).isVisible();
