...
setupProjectPeopleModules({"id":1303,"name":"出勤打卡管理系統（Attendance Management）","description":"記錄產線與辦公室員工出勤刷卡、請假、加班等時間資料，作為薪資核算之基礎依據。","modules":["出勤總覽","打卡異常","假勤簽核","AI 排班建議"],"functions":["多元刷卡方式（門禁卡、指紋、臉部辨識）","請假/加班線上申請","異常出勤（遲到早退曠職）自動標記","彈性工時/責任制規則設定","產線出勤看板即時顯示","出勤報表匯出"],"workflows":["員工刷卡/生物辨識","出勤資料上傳","異常比對","主管確認","資料鎖定","傳送薪資系統"],"pains":["大量作業員代打卡問題","產線缺勤即時掌握困難","各類假別規則繁雜易誤算","出勤異常追蹤耗費人力"],"ai":["臉部辨識防代打卡偵測","異常缺勤模式預測分析"],"roles":["人資部","產線班長/組長","行政部","警衛/門禁單位"],"metrics":["出勤異常率","代打卡攔截數","出勤資料正確率","請假簽核時效"],"fields":["對象","期限","風險","負責人"]});
