...
const url = process.argv[2] || "http://127.0.0.1:3024";

const checks = [
  ["hasModules", "text=功能模組"],
  ["hasQuote", "text=報價估價"],
  ["hasProject", "text=工程進度"],
  ["hasQuality", "text=品質圖說與財務"],
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/estimate-pmis-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
