...
setupProjectPeopleModules({"id":1313,"name":"職業安全衛生管理系統（EHS, Environment, Health & Safety）","description":"管理產線職安衛法規遵循、危害辨識、工安事故通報與健康檢查，降低製程作業風險。","modules":["安衛總覽","危害巡檢","事故與改善","AI 風險預警"],"functions":["危害辨識與風險評估（HIRA）","工安事故/虛驚事件通報","個人防護具（PPE）發放紀錄","健康檢查排程與異常追蹤","化學品/氣體管理（SDS）","職安教育訓練與稽核排程"],"workflows":["危害辨識","風險評估","防護措施執行","事故/異常通報","改善追蹤","稽核結案"],"pains":["工安事故通報延遲難即時處置","化學品安全資料分散管理","健檢異常個案未有效追蹤","承攬商入廠安全管控不易"],"ai":["工安影像辨識（未戴PPE偵測）","事故熱點與高風險工序預測"],"roles":["職安衛部門","產線主管","廠務工程部","人資部"],"metrics":["職災發生率","虛驚事件通報數","健檢異常改善率","PPE配戴合規率"],"fields":["對象","期限","風險","負責人"]});
