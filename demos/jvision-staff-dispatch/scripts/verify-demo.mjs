...
const url = process.argv[2] || "http://127.0.0.1:3022";

const checks = [
  ["hasModules", "text=系統模組"],
  ["hasWorkers", "text=派遣員工管理"],
  ["hasJobs", "text=派工出勤"],
  ["hasPayroll", "text=薪資結算"],
...
for (const viewport of [
  ...
  await page.screenshot({ path: `verification/staff-dispatch-${viewport.name}.png`, fullPage: true });
  results.push(result);
  await page.close();
}
