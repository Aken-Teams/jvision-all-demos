...
const forbidden = ["視界管家", "Optical SaaS", "optic-book", "????", "已改為", "相關字樣"];
...
await page.getByRole("button", { name: "生成 AI 門市摘要" }).click();
await page.getByRole("button", { name: "批次發送 LINE 訊息" }).click();
...
await page.goto(url, { waitUntil: "networkidle", timeout: 60000 });
const mobileHeading = await page.getByRole("heading", { name: "Jvision 眼鏡門市預約會員經營平台" }).isVisible();
